...
select *
from nhan_vien
where ho_ten like 'H%' and char_length(ho_ten) <=15
union
select *
from nhan_vien
where ho_ten like 'T%' and char_length(ho_ten) <=15
union
select *
from nhan_vien
where ho_ten like 'K%' and char_length(ho_ten) <=15;
...
select k.ma_khach_hang, k.ho_ten, l.ten_loai_khach, h.ma_hop_dong, d.ten_dich_vu, h.ngay_lam_hop_dong, h.ngay_ket_thuc, chi_phi_thue+(so_luong*gia) as tong_tien
from loai_khach l
inner join khach_hang k
using (ma_loai_khach)
inner join hop_dong h
using (ma_khach_hang)
inner join dich_vu d
using (ma_dich_vu)
inner join hop_dong_chi_tiet hd
using (ma_hop_dong)
inner join dich_vu_di_kem dv
